...
class _TextLayouter extends _ElementLayouter {
  ...
  PdfTextElement get element =>
      super.element is PdfTextElement ? super.element : null;
  ...
  _TextPageLayoutResult _layoutOnPage(String text, PdfPage currentPage,
    ...
    if (canDraw) {
      ...
      graphics._drawStringLayoutResult(stringResult, element.font, element.pen,
          element._obtainBrush(), currentBounds, _format);
  ...
  Rect _getTextPageBounds(PdfPage currentPage, _Rectangle currentBounds,
    ...
    ArgumentError.checkNotNull(currentPage);
    ArgumentError.checkNotNull(stringResult);
  ...
  void _checkCorectStringFormat(_LineInfo lineInfo) {
    if (_format != null) {
      _format._firstLineIndent = lineInfo._lineType &
                  _PdfStringLayouter._getLineTypeValue(_LineType.newLineBreak) >
              0
