...
class _TtfReader {
  ...
  List<int> _readWidthTable(int glyphCount, int unitsPerEm) {
    ...
    _offset = tableInfo.offset;
    final List<int> width = List<int>(glyphCount);
  ...
  void _initializeMetrics(
    ...
    ArgumentError.checkNotNull(cmapTables);
    _initializeFontName(nameTable);
    bool bSymbol = false;
    ...
    final double top = (_metrics.macAscent + _metrics.lineGap).ceilToDouble();
  ...
  List<_TtfCmapSubTable> _readCmapTable() {
    ...
    int position = _offset;
    final List<_TtfCmapSubTable> subTables =
        List<_TtfCmapSubTable>(table.tablesCount);
  ...
  List<int> _updateWidth() {
    ...
    if (_metrics.isSymbol) {
      final List<int> bytes = List<int>(count);
      for (int i = 0; i < count; i++) {
        final _TtfGlyphInfo glyphInfo = _getGlyph(char: String.fromCharCode(i));
        bytes[i] = (glyphInfo.empty) ? 0 : glyphInfo.width;
      }
      return bytes;
    } else {
      final List<int> bytes = List<int>.filled(count, 0);
      final List<int> byteToProcess = List<int>(1);
  ...
  Map<int, int> _getGlyphChars(Map<String, String> chars) {
    ArgumentError.checkNotNull(chars);
  ...
  dynamic _updateLocaTable(
    ...
    ArgumentError.checkNotNull(newLocaTable);
    final int size =
  ...
  List<int> _getFontProgram(List<int> newLocaTableOut, List<int> newGlyphTable,
    ...
    ArgumentError.checkNotNull(newLocaTableOut);
    ArgumentError.checkNotNull(newGlyphTable);
  ...
  List<int> _readBytes(int length) {
    final List<int> result = List<int>(length);
    for (int i = 0; i < length; i++) {
  ...
  List<int> _readUshortArray(int length) {
    final List<int> buffer = List<int>(length);
    for (int i = 0; i < length; i++) {
