...
class PdfPageLayerCollection extends PdfObjectCollection {
  ...
  void _parseLayers(PdfPage page) {
    ...
    if (!page._isTextExtraction) {
      ...
      if (ocProperties != null && (propertie != null)) {
        ...
        _PdfReferenceHolder layerReferenceHolder;
        if (propertie != null && (propertie != null)) {
          propertie._items.forEach((_PdfName key, _IPdfPrimitive value) {
            layerReferenceHolder = value as _PdfReferenceHolder;
            layerDictionary =
      ...
      if (ocDictionary != null &&
        ...
        if (ocDictionary.containsKey(_DictionaryProperties.defaultView)) {
          final _PdfDictionary defaultView =
              ocDictionary[_DictionaryProperties.defaultView] as _PdfDictionary;
          if (defaultView != null) {
  ...
  _IPdfPrimitive _createOptionContentDictionary(PdfPageLayer layer) {
    ...
    if (!_subLayer) {
      ...
      _page._document._orderPosition++;
    }
    if (layer.visible) {
      _page._document._on._add(reference);
      _page._document._onPosition++;
    } else {
      _page._document._off._add(reference);
      _page._document._offPosition++;
    }
    _page._document._position++;
  ...
  void _checkVisible(_PdfDictionary ocproperties,
    ...
    if (ocproperties != null) {
      ...
      if (defaultView != null) {
        final _PdfArray visible = _PdfCrossTable._dereference(
            defaultView[_DictionaryProperties.ocgOff]) as _PdfArray;
        if (visible != null && layerDictionary.isNotEmpty) {
          for (int i = 0; i < visible.count; i++) {
            if (layerDictionary
                .containsKey(visible[i] as _PdfReferenceHolder)) {
  ...
  void clear() {
    for (final PdfPageLayer layer in _page.layers._list) {
      _removeLayer(layer);
    }
    _list.clear();
  }
  ...
  void _removeContent(_PdfArray content, _PdfReferenceHolder referenceHolder) {
    ...
    for (int i = 0; i < content.count; i++) {
      if (content._elements[i] is _PdfReferenceHolder) {
        if ((content._elements[i] as _PdfReferenceHolder).reference != null &&
            referenceHolder.reference != null) {
          if ((content._elements[i] as _PdfReferenceHolder).reference._objNum ==
        ...
        _removeContent(content._elements[i], referenceHolder);
      }
    }
    if (flag) {
  ...
  void _removeLayerContent(PdfPageLayer layer) {
    ...
    for (int m = 0; m < _page._contents.count; m++) {
      ...
      final _PdfReferenceHolder pdfReference =
          _page._contents[m] as _PdfReferenceHolder;
      if (pdfReference != null && pdfReference.reference == null) {
