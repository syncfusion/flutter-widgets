...
class PdfPageCollection {
  ...
  PdfPageCollection._fromCrossTable(
    ...
    _document = document ?? PdfDocument();
  ...
  PdfPage insert(int index,
    ...
    sec._dropCropBox();
    sec._add(page);
    _PdfDictionary dic = sec._element;
    int localIndex = 0;
  ...
  int _getNodeCount(_PdfDictionary node) {
    final _PdfNumber number =
        _crossTable._getObject(node[_DictionaryProperties.count]) as _PdfNumber;
  ...
  void _addPage(PdfPage page) {
    ...
    if (_section == null) {
      if (section.pageSettings.orientation !=
          _document.pageSettings.orientation) {
        section = _document.sections.add();
        section.pageSettings.orientation = _document.pageSettings.orientation;
      }
  ...
  Map<String, dynamic> _getValidParent(
    ...
    final _IPdfPrimitive obj = _document._catalog[_DictionaryProperties.pages];
    ...
    if (index == 0 && !zeroValid) {
      ...
      for (int i = 0; i < kids.count; i++) {
        final _PdfReferenceHolder pageReferenceHolder =
            kids._elements[i] as _PdfReferenceHolder;
        if (pageReferenceHolder != null) {
  ...
  Map<String, dynamic> _getParent(int index, int localIndex) {
    ...
    if (_nodeDictionary == null) {
      _nodeDictionary = _crossTable._getObject(_pageCatalog) as _PdfDictionary;
  ...
  void removeAt(int index) {
    ArgumentError.checkNotNull(index, 'index value cannot be null');
    ...
    if (_document._isLoadedDocument && index > -1) {
        ...
        if (bookmarks != null) {
          for (int i = 0; i < bookmarks.length; i++) {
            if (bookmarks[i] is PdfBookmarkBase) {
              ...
              if (current._dictionary.containsKey(_DictionaryProperties.dest)) {
      ...
      if (index == 0) {
        ...
        if (table.documentCatalog != null) {
          final _IPdfPrimitive primitive = table.documentCatalog['OpenAction'];
      ...
      if (remove != null) {
        ...
        if (kids.count == 0 &&
          ...
          if (holder is _PdfReferenceHolder) {
            holder = (holder as _PdfReferenceHolder)._object;
          ...
          if (parentDic != null) {
            ...
            if (kidsPrimitive is _PdfReferenceHolder) {
              kidsPrimitive = (kidsPrimitive as _PdfReferenceHolder)._object;
