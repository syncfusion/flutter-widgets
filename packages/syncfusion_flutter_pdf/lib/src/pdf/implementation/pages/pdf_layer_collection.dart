...
class PdfLayerCollection extends PdfObjectCollection {
    ...
    if (_sublayer == false) {
      ...
      if (_document._order._contains(_parent._referenceHolder)) {
        ...
        if (_parent._parent != null) {
          if (_parent._parent._sublayer._contains(_parent._referenceHolder)) {
            int position =
                _parent._parent._sublayer._indexOf(_parent._referenceHolder);
            if (_parent._sublayer.count == 1) {
              _parent._parent._sublayer
                  ._insert(position + 1, _parent._sublayer);
            }

            if (_document._order._contains(_parent._parent._referenceHolder)) {
              position =
                  _document._order._indexOf(_parent._parent._referenceHolder);
              _document._order._removeAt(position + 1);
              _document._order._insert(position + 1, _parent._parent._sublayer);
            }
          }
        }
      }
                ...
                if (usage != null) {
                  final _PdfDictionary printOption =
                      _PdfCrossTable._dereference(
                          usage[_DictionaryProperties.print]) as _PdfDictionary;
    ...
    if (_document._catalog.containsKey(_DictionaryProperties.ocProperties)) {
      ...
      if (visible != null) {
        for (int i = 0; i < visible.count; i++) {
          final PdfLayer pdfLayer = _layerDictionary[visible[i]];
          if (pdfLayer != null) {
  ...
  void _checkLayerLock(_PdfDictionary ocProperties) {
    ...
    if (locked != null) {
      for (int i = 0; i < locked.count; i++) {
        final PdfLayer pdfLayer = _layerDictionary[locked[i]];
        if (pdfLayer != null) {
          continue;
        }
      }
  ...
  void _createLayerHierarchical(_PdfDictionary ocProperties) {
    ...
    if (defaultView != null &&
      ...
      if (_layerDictionary != null && _layerDictionary.isNotEmpty) {
        _list.clear();
    ...
    if (order != null) {
      for (int i = 0; i < order.count; i++) {
        if (order[i] is _PdfReferenceHolder) {
          final _PdfReferenceHolder holder = order[i];
  ...
  void _removeOCProperties(
    ...
    for (int i = 0; i < content.count; i++) {
      if (content._elements[i] is _PdfReferenceHolder) {
        ...
        _removeOCProperties(content._elements[i], referenceHolder);
      }
    }
    if (isChange) {
