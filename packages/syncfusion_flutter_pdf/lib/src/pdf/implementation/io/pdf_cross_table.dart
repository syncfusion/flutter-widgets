...
class _PdfCrossTable {
  ...
  Queue<_PdfReference> _objNumbers;
  ...
  void _saveIndirectObject(_IPdfPrimitive object, _PdfWriter writer) {
    ...
    if (object is _PdfDictionary &&
      ...
      if (dictionary.containsKey(_DictionaryProperties.type)) {
        if (dictionary[_DictionaryProperties.type] as _PdfName ==
            _PdfName('Sig')) {
          sigFlag = true;
        }
      }
  ...
  _PdfReference _getReference(_IPdfPrimitive object) {
    ...
    if (object is _PdfReferenceHolder) {
      object = (object as _PdfReferenceHolder).object;
    ...
    if (reference == null) {
      ...
      if (_crossTable != null && _crossTable._objects != null) {
        while (_crossTable._objects.containsKey(objectNumber)) {
          objectNumber = nextObjectNumber;
        }
      }
      if (_document._objects._mainObjectCollection.containsKey(objectNumber)) {
        reference = _PdfReference(nextObjectNumber, 0);
      } else {
        _PdfNumber trailerCount;
        if (_crossTable != null) {
          trailerCount = _crossTable._trailer[_DictionaryProperties.size];
        }
      ...
      if (wasNew) {
        ...
        final int tempkey =
            _document._objects._objectCollection[tempIndex]._reference._objNum;
        final _ObjectInfo tempvalue =
            _document._objects._objectCollection[_document._objects._count - 1];
  ...
  void _saveSubSection(
    ...
    for (double i = objectNum; i < objectNum + count; ++i) {
      final _RegisteredObject obj = _objects[i];
...
class _RegisteredObject {
  _RegisteredObject(int offset, _PdfReference reference, [bool isFreeType]) {
    if (reference == null) {
      throw ArgumentError.notNull('reference');
    } else {
      _offset = offset;
