...
class _CrossTable {
  ...
  _PdfReader _reader;
  _PdfParser _parser;
  Map<int, _ObjectInformation> _objects;
  Map<_PdfStream, _PdfParser> _readersTable;
  Map<int, _PdfStream> _archives;
  ...
  void _initialize() {
    ...
    while (trailer.containsKey(_DictionaryProperties.prev)) {
      if (_whiteSpace != 0) {
        (trailer[_DictionaryProperties.prev] as _PdfNumber).value +=
            _whiteSpace;
  ...
  void _parseSubsection(_PdfParser parser, Map<int, _ObjectInformation> table) {
