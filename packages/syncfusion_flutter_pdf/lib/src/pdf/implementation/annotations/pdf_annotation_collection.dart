...
class PdfAnnotationCollection extends PdfObjectCollection
  ...
  // Properties
  /// Gets the annotation flatten.
  bool get flatten => _flatten;

  /// Sets the annotation flatten.
  set flatten(bool value) {
    ArgumentError.checkNotNull(value, 'flatten can be set to null');
    _flatten = value;
  ...
  PdfAnnotation operator [](int index) {
    ...
    final PdfAnnotation annotation = _list[index];
    if (!_page._isLoadedPage) {
      return annotation;
    } else {
      annotation != null && annotation._isLoadedAnnotation
    ...
    if (!result) {
      for (int i = 0; i < array._elements.length; i++) {
        if (array._elements[i] is _PdfReferenceHolder) {
          final _PdfReferenceHolder holder = array._elements[i];
  ...
  void _doRemove(PdfAnnotation annot) {
    if (_page._isLoadedPage) {
      ...
      annot._dictionary
          .setProperty(_DictionaryProperties.p, _PdfReferenceHolder(_page));
  ...
  PdfAnnotation _getAnnotation(int index) {
    ...
    PdfAnnotation annot;
    if (dictionary != null &&
        dictionary.containsKey(_DictionaryProperties.subtype)) {
          ...
          if (name != null) {
            final _PdfArray border = (_PdfCrossTable._dereference(
                ...
                ? _PdfCrossTable._dereference(
                    dictionary[_DictionaryProperties.border])
                : null;
  ...
  bool _findAnnotation(_PdfArray arr) {
    ...
    for (int i = 0; i < arr.count; i++) {
      if (arr[i] is _PdfArray) {
        final _PdfArray temp = arr[i];
        for (int j = 0; j < temp.count; j++) {
          final _PdfNumber value = (temp[j] is _PdfNumber) ? temp[j] : null;
          int val = 0;
          if (value != null) {
            val = value.value;
          }
        ...
        int val = 0;
        final _PdfNumber value = (arr[i] is _PdfNumber) ? arr[i] : null;
        if (value != null) {
