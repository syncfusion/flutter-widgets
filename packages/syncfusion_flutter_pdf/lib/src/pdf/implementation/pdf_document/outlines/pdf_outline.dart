...
class PdfBookmark extends PdfBookmarkBase {
  ...
  /// Gets the outline destination page.
  ...
  /// Sets the outline destination page.
  set destination(PdfDestination value) {
    ArgumentError.checkNotNull(value, 'destination');
    _destination = value;
    _dictionary.setProperty(_DictionaryProperties.dest, value);
  }

  /// Gets the named destination in outline.
  ...
  /// Sets the named destination in outline.
  set namedDestination(PdfNamedDestination value) {
    if (_namedDestination != value) {
  ...
  set title(String value) {
    ArgumentError.checkNotNull(value, 'title');
    _dictionary[_DictionaryProperties.title] = _PdfString(value);
  }

  /// Gets the color of bookmark title.
  PdfColor get color {
    if (_isLoadedBookmark) {
      return _obtainColor();
    } else {
  ...
  /// Sets the color of bookmark title.
  ...
  /// Gets the style of the outline title.
  ...
  /// Sets the style of the outline title.
  set textStyle(List<PdfTextStyle> value) {
    if (_isLoadedBookmark) {
      _assignTextStyle(value);
    } else {
      ArgumentError.checkNotNull(value, 'textStyle');
      if (_textStyle != value) {
        _textStyle = value;
        _updateTextStyle();
      }
  ...
  set _previous(PdfBookmark value) {
    if (_previousBookmark != value) {
      ...
      _dictionary.setProperty(
          _DictionaryProperties.prev, _PdfReferenceHolder(value));
    }
  }
  ...
  set _next(PdfBookmark value) {
    if (_nextBookmark != value) {
      _nextBookmark = value;
      _dictionary.setProperty(
          _DictionaryProperties.next, _PdfReferenceHolder(value));
    }
  }
  ...
  void _updateColor() {
    final _PdfArray array = _dictionary[_DictionaryProperties.c] as _PdfArray;
    if (array != null && _color.isEmpty) {
      _dictionary.remove(_DictionaryProperties.c);
    } else {
      _dictionary[_DictionaryProperties.c] = _color._toArray(PdfColorSpace.rgb);
    }
  }
  ...
  PdfNamedDestination _obtainNamedDestination() {
    ...
    if (namedCollection != null) {
      ...
      if (destination != null) {
        ...
        final _PdfString str = (destination is _PdfString) ? destination : null;
        String title;
        if (name != null) {
          title = name._name;
        } else if (str != null) {
  ...
  PdfDestination _obtainDestination() {
    if (_dictionary.containsKey(_DictionaryProperties.dest) &&
      ...
      if (obj is _PdfReferenceHolder) {
        obj = (obj as _PdfReferenceHolder)._object;
      }
  ...
  double _checkRotation(PdfPage page, _PdfNumber top, _PdfNumber left) {
    ...
    if (page._rotation == PdfPageRotateAngle.rotateAngle90) {
      topValue = (top == null) ? 0 : left.value;
    } else if (page._rotation == PdfPageRotateAngle.rotateAngle180) {
      topValue = (top == null) ? 0 : top.value;
