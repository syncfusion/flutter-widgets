...
class PdfBookmarkBase implements _IPdfWrapper {
  ...
  PdfBookmarkBase._load(_PdfDictionary dictionary, _PdfCrossTable crossTable) {
  ...
  bool get _isExpanded {
    if (_dictionary.containsKey('Count')) {
      return (_dictionary[_DictionaryProperties.count] as _PdfNumber).value < 0
          ? false
          : true;
    } else {
      return _expanded;
  ...
  void remove(String title) {
    ...
    if (_bookmark == null || _bookmark.length < _list.length) {
      ...
      for (int n = 0; n < _list.length; n++) {
        ArgumentError.checkNotNull(_list[n] as PdfBookmark, 'bookmark');
