...
class _X509Name extends _Asn1Encode {
  _X509Name(_Asn1Sequence sequence) {
    _initialize();
    _sequence = sequence;
    sequence._objects!.forEach((encode) {
  ...
  List<dynamic> _ordering = <dynamic>[];
  List<dynamic> _values = <dynamic>[];
  List<dynamic> _added = <dynamic>[];
  Map<_DerObjectID, String> _defaultSymbols = <_DerObjectID, String>{};
  ...
  _Asn1? getAsn1() {
    if (_sequence == null) {
      ...
      for (int i = 0; i != _ordering.length; i++) {
        final _DerObjectID? oid = _ordering[i];
        if (lstOid != null && !(_added[i])) {
