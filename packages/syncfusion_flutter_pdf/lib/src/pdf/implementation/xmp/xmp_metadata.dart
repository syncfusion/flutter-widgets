...
class _XmpMetadata extends _IPdfWrapper {
  ...
  _XmpMetadata.fromXmlDocument(XmlDocument xmp) {
    ArgumentError.checkNotNull(xmp, 'xmpMetadata');
    _stream = _PdfStream();
  ...
  void load(XmlDocument xmp) {
    ArgumentError.checkNotNull(xmp, 'xmp');
    _reset();
    _xmlData = xmp;
  ...
  void _createRdf(PdfDocumentInformation info) {
    ...
    if (!_isNullOrEmpty(info.creator) ||
        info._creationDate != null ||
        info._modificationDate != null) {
      ...
      if (info._creationDate != null) {
        final String createDate = _getDateTime(info._creationDate);
        xmpDescription.children.add(XmlElement(
            XmlName('CreateDate', 'xmp'), [], [XmlText(createDate)]));
      }
      if (info._modificationDate != null && !info._isRemoveModifyDate) {
  ...
  XmlElement _createElement(
    ...
    ArgumentError.checkNotNull(localName, 'localName');
    XmlElement element;
  ...
  String _addNamespace(String prefix, String namespaceURI) {
    ArgumentError.checkNotNull(prefix, 'prefix');
    ArgumentError.checkNotNull(namespaceURI, 'namespaceURI');
