...
class _PdfGridLayouter extends _ElementLayouter {
  ...
  _Size _currentPageBounds;
  _Rectangle _currentBounds;
  _Point _startLocation;
  double _childHeight;
  PdfHorizontalOverflowType _hType;
  List<List<int>> _columnRanges;
  int _cellEndIndex;
  int _cellStartIndex;
  static int _repeatRowIndex = -1;
  List<int> _parentCellIndexList;
  int _currentRowIndex;
  int _rowBreakPageHeightCellIndex;
  bool flag;
  bool _isChanged;
  _Point _currentLocation;
  bool _isChildGrid;
  double _newheight;
  ...
  PdfLayoutResult _layoutOnPage(_PdfLayoutParams param) {
    ...
    PdfGridEndPageLayoutArgs endArgs;
      ...
      for (int rowIndex = 0; rowIndex < _grid.rows.count; rowIndex++) {
        ...
        while (!rowResult.isFinish && startPage != null) {
          ...
          if (startPage != _currentPage) {
            if (row._grid._isChildGrid && row._grid._parentCell != null) {
              ...
              if (row._grid._parentCell._row._grid.style.cellPadding != null) {
                bounds.y +=
          ...
          if (repeatRow) {
            ...
            if (row._grid._parentCell != null) {
              ...
              row._grid._parentCell._row._rowBreakHeight = row._rowBreakHeight +
                  _grid._parentCell._row._grid.style.cellPadding.top +
        ...
        if (!rowResult.isFinish &&
          ...
          if (!_grid._isSingleGrid) {
            for (int j = 0; j < _grid.rows.count; j++) {
              ...
              if (isWidthGreaterthanParent &&
                  _grid.rows[j].cells[_rowBreakPageHeightCellIndex]._pageCount >
                      0) {
                isAddNextPage = true;
              }
            }
          ...
          if (_currentBounds.x == _grid._defaultBorder.left.width / 2) {
            _currentBounds.y += _startLocation.x;
          }
        ...
        if (row._gridResult != null) {
          ...
          if (startPage != _currentPage) {
            ...
            for (int page = startIndex; page < endIndex + 1; page++) {
              ...
              location.y = format == null ? 0.5 : format.paginateBounds.top;
              for (int c = 0; c < row.cells.count; c++) {
                final PdfGridCell cell = row.cells[c];
                double cellWidth = 0.0;
                if (cell.columnSpan > 1) {
        ...
        if (grid.rows[j].cells[_cellStartIndex].value is PdfGrid) {
          ...
          if (childGrid == _grid) {
            if (!_isChildGrid) {
              ...
              if (j == 0) {
                _currentBounds.y -= grid._size.height;
              } else {
                int k = j;
                while (k < grid.rows.count) {
                  _currentBounds.y -= grid.rows[k].height;
                  k++;
                }
              }
            }
            ...
            _currentBounds.x +=
                grid.style.cellPadding.left + grid.style.cellPadding.right;
            _currentBounds.y +=
                grid.style.cellPadding.top + grid.style.cellPadding.bottom;
            _currentBounds.width -= 2 * _currentBounds.x;
            break;
          } else {
            _isChildGrid = true;
            ...
            if (!isPresent) {
              _currentBounds.y -= childGrid._size.height;
            }
            _isChildGrid = false;
            break;
          }
          ...
          for (int cellIndex = 0; cellIndex < row.cells.count; cellIndex++) {
            ...
            for (int i = currRowIndex; i < currRowIndex + maxSpan; i++) {
              ...
              if ((_currentBounds.y + rowHeightWithSpan) >
                ...
                rowHeightWithSpan -=
                    isHeader ? _grid.headers[i].height : _grid.rows[i].height;
                for (int j = 0; j < _grid.rows[currRowIndex].cells.count; j++) {
        ...
        if (height +
          ...
          if (_grid.allowRowBreakingAcrossPages) {
            ...
            if (_grid._isChildGrid && row._rowBreakHeight > 0) {
              if (_grid._parentCell._row._grid.style.cellPadding != null) {
                _currentBounds.y +=
                    _grid._parentCell._row._grid.style.cellPadding.top;
              }
              _currentBounds.x = _startLocation.x;
      ...
      for (int i = _cellStartIndex; i <= _cellEndIndex; i++) {
        ...
        if (row.cells[i].value is PdfGrid) {
          ...
          if (_grid.columns[i].width >= _currentGraphics.clientSize.width) {
            ...
            location.x += _grid.columns[i].width;
          }
        } else {
          location.x += _grid.columns[i].width;
        }
  ...
  Map<String, dynamic> _raiseBeforePageLayout(
    ...
    if (_element._raiseBeginPageLayout) {
      ...
      if (_Rectangle.fromRect(currentBounds) !=
        ...
        _grid._measureColumnsWidth(_Rectangle(args.bounds.left, args.bounds.top,
            args.bounds.width + args.bounds.left, args.bounds.height));
      }
      cancel = args.cancel;
  ...
  void _raiseAfterCellLayout(
    ...
    if (_grid._raiseEndCellLayout) {
      args = PdfGridEndCellLayoutArgs._(
          graphics, rowIndex, cellIndex, bounds, value, cellstyle, isHeaderRow);
  ...
  PdfLayoutResult _layoutInternal(_PdfLayoutParams param) {
    ...
    if (_grid.rows.count != 0) {
      _currentBounds.width = (param.bounds.width > 0)
          ? param.bounds.width
          : _grid.rows[0].cells[0].style.borders != null &&
                  _grid.rows[0].cells[0].style.borders.left != null
      ...
      _currentBounds.width = (param.bounds.width > 0)
          ? param.bounds.width
          : _grid.headers[0].cells[0].style.borders != null &&
                  _grid.headers[0].cells[0].style.borders.left != null
              ? (_currentBounds.width -
