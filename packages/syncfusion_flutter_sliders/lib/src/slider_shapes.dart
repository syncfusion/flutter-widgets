...
class SfTrackShape {
  ...
  void _drawSliderTrack(
    ...
    if (!isVertical) {
      ...
      inactiveTrackRRect = RRect.fromRectAndCorners(
        trackRect,
        topRight: radius,
        bottomRight: radius,
      );
  ...
  void _drawRangeSliderTrack(
    ...
    if (!isVertical) {
      ...
      // Drawing active track.
      final Rect activeTrackRRect = Rect.fromLTRB(
        startThumbCenter.dx,
        activeTrackRect.top,
        endThumbCenter.dx,
        activeTrackRect.bottom,
...
class SfThumbShape {
  ...
  void paint(
    ...
    final bool hasThumbStroke =
        themeData.thumbStrokeColor != null &&
...
class SfPaddleTooltipShape extends SfTooltipShape {
  ...
  void _drawPaddleTooltip(
    ...
    final double thumbRadius =
        (parentBox as RenderBaseSlider).thumbShape
    ...
    final Offset topNeckCenter = Offset(
      topNeckRadius + neckDifference / 2,
      -offset.dy - bottomNeckRadius,
    );
    ...
    textPainter.paint(
      context.canvas,
      Offset(
        -textPainter.width / 2 - shiftPaddleWidth,
  ...
  Path _getPaddleTooltipPath(
    ...
    path.moveTo(
      neckDifference / 2,
      topNeckCenter.dy + topNeckRadius * moveNeckValue,
    );
    // Drawn top paddle shape.
    path.arcTo(
      Rect.fromCircle(
        center: Offset(topNeckCenter.dx, topNeckCenter.dy + moveRightTopNeckY),
    ...
    path.arcTo(
      Rect.fromCircle(
        center: Offset(
          paddleTopCircleCenter.dx - shiftPaddleWidth,
    ...
    path.arcTo(
      Rect.fromCircle(
        center: Offset(-topNeckCenter.dx, topNeckCenter.dy + moveLeftTopNeckY),
        radius: topNeckRadius,
      ),
    ...
    path.arcTo(
      Rect.fromCircle(center: Offset.zero, radius: thumbRadius),
      3 * math.pi / 2,
      -math.pi,
      false,
    );
