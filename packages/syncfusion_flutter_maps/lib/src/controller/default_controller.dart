...
class MapController {
  ...
  double tileLayerZoomLevel;
  MapZoomLevel tileCurrentLevelDetails;
  Size totalTileSize;
  Size tileLayerBoxSize;
  bool isTileLayerChild = false;
  ...
  Size getTileSize({double zoomLevel}) {
    zoomLevel ??= tileLayerZoomLevel;
    return Size.square(256 * pow(2, zoomLevel));
  }

  void applyTransform(PaintingContext context, Offset offset,
      [bool isVectorLayer = false]) {
    if (isVectorLayer && tileCurrentLevelDetails != null) {
      context.canvas
